...
const char * emulator_input_cheatlabel(void)
{
	...
	const uint64_t sa1_enabled = Settings.SA1;
	const uint64_t superfx_enabled = Settings.SuperFX;
...
static void emulator_start(void)
{
	...
	if(sa1_enabled)
	{
		emulator_implementation_s9x_sa1_mainloop(); //SA1- game
	}
	else if(superfx_enabled)
	{
		//SuperFX - game
		if(Settings.CurrentROMisMouseCompatible)
		{
			if(Settings.AccessoryType)
			{
				//SuperFX - game with mouse peripheral support
				emulator_implementation_s9x_superfx_mainloop_mouse();
			}
			else
			{
				emulator_implementation_s9x_superfx_mainloop_analog();
			}
		}
		else
		{
			//SuperFX - game with standard controls
			emulator_implementation_s9x_superfx_mainloop();
		}
	}
	else
	{
		...
		if(Settings.CurrentROMisMouseCompatible)
		{
			//Game - mouse compatible
			if(Settings.AccessoryType)
			{
				emulator_s9x_mainloop_mouse();
			}
			else
			{
				emulator_s9x_mainloop_analog(); 
			}
		}
		else if(Settings.CurrentROMisSuperScopeCompatible)
		{
			//Game - Super Scope compatible
			if(Settings.AccessoryType)
			{
				emulator_s9x_mainloop_scope();
			}
			else
			{
				emulator_s9x_mainloop_analog(); 
