...
void ps3graphics_draw(int width, int height, uint16_t* screen)
{
	...
	if (fbo_enable)
	{
		...
		glBindTexture(GL_TEXTURE_2D, tex);
		glBufferSubData(GL_TEXTURE_REFERENCE_BUFFER_SCE, 0, height << 10, screen);
		glTextureReferenceSCE(GL_TEXTURE_2D, 1, width, height, 0, GL_RGB5_A1, SCREEN_RENDER_TEXTURE_PITCH, 0);
		...
		UpdateCgParams(width, height, fbo_vp_width, fbo_vp_height, 0);
	...
	else
	{
		...
		//going to hardcode SNES9x_SCREEN_PITCH HERE since integer multiplication is microcoded - 1024 
		//left shift immediate by constant 10
		glBufferSubData(GL_TEXTURE_REFERENCE_BUFFER_SCE, 0, height << 10, screen);
		glTextureReferenceSCE(GL_TEXTURE_2D, 1, width, height, 0, GL_RGB5_A1, SCREEN_RENDER_TEXTURE_PITCH, 0);

		UpdateCgParams(width, height, width, height, 0);
