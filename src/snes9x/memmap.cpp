...
static bool8 ReadUPSPatch_unzreader (unzReader *, long, int32 &);
static bool8 ReadUPSPatch_freader (fReader *, long, int32 &);
static long ReadInt_freader (fReader *, unsigned);
static long ReadInt_unzreader (unzReader *, unsigned);
static bool8 ReadIPSPatch_unzreader (unzReader *, long, int32 &);
static bool8 ReadIPSPatch_freader (fReader *, long, int32 &);
#ifdef UNZIP_SUPPORT
...
static void S9xDeinterleaveType2 (int size, uint8 *base)
{
	...
	Settings.DisplayColor = (((int) (31) << 10) | ((int) (14) << 5) | (int) (6));
...
static void S9xDeinterleaveGD24 (int size, uint8 *base)
{
	...
	Settings.DisplayColor = (((int) (0) << 10) | ((int) (31) << 5) | (int) (31));
...
bool8 CMemory::Init (void)
{
	...
	IPPU.TileCache[TILE_2BIT]       = (uint8 *) malloc(MAX_2BIT_TILES << 6);
	IPPU.TileCache[TILE_4BIT]       = (uint8 *) malloc(MAX_4BIT_TILES << 6);
	IPPU.TileCache[TILE_8BIT]       = (uint8 *) malloc(MAX_8BIT_TILES << 6);
	IPPU.TileCache[TILE_2BIT_EVEN]  = (uint8 *) malloc(MAX_2BIT_TILES << 6);
	IPPU.TileCache[TILE_2BIT_ODD]   = (uint8 *) malloc(MAX_2BIT_TILES << 6);
	IPPU.TileCache[TILE_4BIT_EVEN]  = (uint8 *) malloc(MAX_4BIT_TILES << 6);
	IPPU.TileCache[TILE_4BIT_ODD]   = (uint8 *) malloc(MAX_4BIT_TILES << 6);
	...
	ZeroMemory(IPPU.TileCache[TILE_2BIT],       MAX_2BIT_TILES << 6);
	ZeroMemory(IPPU.TileCache[TILE_4BIT],       MAX_4BIT_TILES << 6);
	ZeroMemory(IPPU.TileCache[TILE_8BIT],       MAX_8BIT_TILES << 6);
	ZeroMemory(IPPU.TileCache[TILE_2BIT_EVEN],  MAX_2BIT_TILES << 6);
	ZeroMemory(IPPU.TileCache[TILE_2BIT_ODD],   MAX_2BIT_TILES << 6);
	ZeroMemory(IPPU.TileCache[TILE_4BIT_EVEN],  MAX_4BIT_TILES << 6);
	ZeroMemory(IPPU.TileCache[TILE_4BIT_ODD],   MAX_4BIT_TILES << 6);
...
{
	uint32	calc_size = (size / 0x2000) << 13;
...
again:
	Settings.DisplayColor = (((int) (31) << 10) | ((int) (31) << 5) | (int) (31));
	...
	CalculatedSize = (totalFileSize / 0x2000) << 13;
	...
	//BEGIN OF SNES9x PS3
	int ret; 
	ret = S9xLoadCheatFile(S9xGetFilename(".cht", CHEAT_DIR));
	//END OF SNES9x PS3
	...
	{
		FILE * fp;
		fp = fopen(S9xGetFilename(".cht", CHEAT_DIR), "w");
		fclose(fp);
		ret = S9xLoadCheatFile(S9xGetFilename(".cht", CHEAT_DIR));
...
bool8 CMemory::LoadSRAM (const char *filename)
{
	...
	size = SRAMSize ? (1 << (SRAMSize + 3)) << 7 : 0;
	if (size > 0x20000)
		size = 0x20000;
...
void CMemory::ParseSNESHeader (uint8 *RomHeader)
{
	...
	if (RomHeader[0x2A] != 0x33)
		CompanyId = mul<36>(((RomHeader[0x2A] >> 4) & 0x0F)) + (RomHeader[0x2A] & 0x0F);
	else
...
void CMemory::InitROM (void)
{
	...
	// SRAM size
	SRAMMask = SRAMSize ? ((1 << (SRAMSize + 3)) << 7) - 1 : 0;

	// checksum
	if (!isChecksumOK || ((uint32) CalculatedSize > (uint32) (((1 << (ROMSize - 7)) << 7) << 10)))
	{
		Settings.DisplayColor = (((int) (31) << 10) | ((int) (31) << 5) | (int) (0));
		SET_UI_COLOR(255, 255, 0);
	}

	if (Multi.cartType == 4)
	{
		Settings.DisplayColor = (((int) (0) << 10) | ((int) (16) << 5) | (int) (31));
...
void CMemory::ApplyROMFixes (void)
{
	...
	{
		Settings.DisplayColor = (((int) (31) << 10) | ((int) (0) << 5) | (int) (0));
	...
	if (!Settings.DisableGameSpecificHacks)
	{
		...
		if (match_na ("EARTHWORM JIM 2")						|| // Earthworm Jim 2
         ...
         match_na ("BATTLETECH 3050")                 || // Battle Tech 3050
         match_na ("MECHWARRIOR 3050")                || // Mechwarrior 3050
	...
	if (!Settings.DisableGameSpecificHacks)
	{
		...
		if (match_na("TRAVERSE") || match_na("BATTLE BLAZE")) // Traverse - Starlight & Prairie
		{
...
static bool8 ReadUPSPatch_freader (fReader *r, long, int32 &rom_size)
{
...
static bool8 ReadUPSPatch_unzreader (unzReader *r, long, int32 &rom_size)
{
	//Reader lacks size() and rewind(), so we need to read in the file to get its size
	uint8 *data = new uint8[8 * 1024 * 1024];  //allocate a lot of memory, better safe than sorry ...
	uint32 size = 0;
...
static long ReadInt_freader (fReader *r, unsigned nbytes)
{
	long	v = 0;
...
static long ReadInt_unzreader (unzReader *r, unsigned nbytes)
{
	long	v = 0;
...
static bool8 ReadIPSPatch_freader (fReader *r, long offset, int32 &rom_size)
{
...
static bool8 ReadIPSPatch_unzreader (unzReader *r, long offset, int32 &rom_size)
{
	const int32	IPS_EOF = 0x00454F46l;
	int32		ofs;
	char		fname[6];
...
void CMemory::CheckForAnyPatch (const char *rom_filename, bool8 header, int32 &rom_size)
{
	...
	if ((patch_file = OPEN_STREAM(fname, "rb")) != NULL)
	{
		...
		ret = ReadUPSPatch_freader(new fReader(patch_file), 0, rom_size);
	...
	if (!strcasecmp(ext, "zip") || !strcasecmp(ext, ".zip"))
	{
		...
		if (file)
		{
			...
			if (port == UNZ_OK)
			{
				...
				ret = ReadUPSPatch_unzreader(new unzReader(file), offset, rom_size);
	...
	if ((patch_file = OPEN_STREAM(fname, "rb")) != NULL)
	{
		...
		ret = ReadIPSPatch_freader(new fReader(patch_file), offset, rom_size);
		CLOSE_STREAM(patch_file);
	...
	if (!strcasecmp(ext, "zip") || !strcasecmp(ext, ".zip"))
	{
		...
		if (file)
		{
			...
			while (port == UNZ_OK)
			{
				...
				ret = ReadIPSPatch_unzreader(new unzReader(file), offset, rom_size);
				unzCloseCurrentFile(file);
