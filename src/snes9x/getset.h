...
#define addCyclesInMemoryAccess_SuperFX \
	if (!CPU.InDMAorHDMA) \
	{ \
		CPU.Cycles += speed; \
		while (CPU.Cycles >= CPU.NextEvent) \
...
#define addCyclesInMemoryAccess_x2_SuperFX \
	if (!CPU.InDMAorHDMA) \
	{ \
		CPU.Cycles += speed << 1; \
		while (CPU.Cycles >= CPU.NextEvent) \
...
inline uint8 S9xGetByte_SuperFX (uint32 Address)
{
	...
	if (GetAddress >= (uint8 *) CMemory::MAP_LAST)
	{
		...
		addCyclesInMemoryAccess_SuperFX;
		return (byte);
	}
	...
	switch ((pint) GetAddress)
	{
		case CMemory::MAP_CPU:
			byte = S9xGetCPU_SuperFX(Address & 0xffff);
			addCyclesInMemoryAccess_SuperFX;
			return (byte);
...
inline uint8 S9xGetByte_NoSA1 (uint32 Address)
{
	int		block = (Address & 0xffffff) >> MEMMAP_SHIFT;
	uint8	*GetAddress = Memory.Map[block];
	int32	speed = memory_speed(Address);
...
inline uint16 S9xGetWord_WrapBank(uint32 Address)
{
	uint32	mask = MEMMAP_MASK & (0xffff);
...
inline uint16 S9xGetWord_WrapPage(uint32 Address)
{
	uint32	mask = MEMMAP_MASK & 0xff;
	if ((Address & mask) == mask)
	{
		PC_t	a;
...
inline uint16 S9xGetWord_WrapNone_SuperFX(uint32 Address)
{
	uint32	mask = MEMMAP_MASK & 0xffffff;
	if ((Address & mask) == mask)
	{
		//PC_t	a;

		OpenBus = S9xGetByte_SuperFX(Address);
	...
	{
		case CMemory::MAP_CPU:
			word  = S9xGetCPU_SuperFX(Address & 0xffff);
			addCyclesInMemoryAccess;
			word |= S9xGetCPU_SuperFX((Address + 1) & 0xffff) << 8;
			addCyclesInMemoryAccess;
			return (word);
		...
		case CMemory::MAP_PPU:
			...
			{
				OpenBus = S9xGetByte_SuperFX(Address);
				return (OpenBus | (S9xGetByte_SuperFX(Address + 1) << 8));
			}

			word  = S9xGetPPU_SuperFX(Address & 0xffff);
			addCyclesInMemoryAccess;
			word |= S9xGetPPU_SuperFX((Address + 1) & 0xffff) << 8;
			addCyclesInMemoryAccess;
			return (word);
...
inline uint16 S9xGetWord_WrapNone_NoSA1(uint32 Address)
{
	uint32	mask = MEMMAP_MASK & 0xffffff;
	if ((Address & mask) == mask)
	{
...
inline void S9xSetByte_SuperFX (uint8 Byte, uint32 Address)
{
#ifdef CPU_SHUTDOWN
	...
	{
		case CMemory::MAP_CPU:
			S9xSetCPU_SuperFX(Byte, Address & 0xffff);
			addCyclesInMemoryAccess_SuperFX;
			return;
		...
		case CMemory::MAP_PPU:
			...
			S9xSetPPU_SuperFX(Byte, Address & 0xffff);
			addCyclesInMemoryAccess_SuperFX;
			return;
...
inline void S9xSetByte_SA1(uint8 Byte, uint32 Address)
{
#ifdef CPU_SHUTDOWN
	CPU.WaitAddress = 0xffffffff;
#endif
	...
	if (SetAddress >= (uint8 *) CMemory::MAP_LAST)
	{
		...
		*SetAddress = Byte;
		...
		/*
		   ...
		   {
		 */
		int32_t condition = SetAddress == SA1.WaitByteAddress1 || SetAddress == SA1.WaitByteAddress2;
		int32_t mask = ((condition) | -(condition)) >> 31;
		uint32_t result_sa1_executing = (((SA1.S9xOpcodes != NULL) & mask) | ((SA1.Executing) & ~mask));
		uint32_t result_sa1_waitcounter = (((0) & mask) | ((SA1.WaitCounter) & ~mask)); 
		SA1.Executing = result_sa1_executing;
		SA1.WaitCounter = result_sa1_waitcounter;
#if 0
...
inline void S9xSetByte_NoSA1 (uint8 Byte, uint32 Address)
{
#ifdef CPU_SHUTDOWN
	CPU.WaitAddress = 0xffffffff;
#endif
	...
	{
		case CMemory::MAP_CPU:
			S9xSetCPU_NoSA1(Byte, Address & 0xffff);
			addCyclesInMemoryAccess;
			return;
...
inline void S9xSetWord_SuperFX_Write10 (uint16 Word, uint32 Address, enum s9xwrap_t w = WRAP_NONE)
{
	uint32	mask = MEMMAP_MASK & (w == WRAP_PAGE ? 0xff : (w == WRAP_BANK ? 0xffff : 0xffffff));
	if ((Address & mask) == mask)
	{
		...
		switch (w)
		{
			case WRAP_PAGE:
				a.xPBPC = Address;
				a.B.xPCl++;
				S9xSetByte_SuperFX(Word >> 8, a.xPBPC);
				break;
			...
			case WRAP_NONE:
			default:
				S9xSetByte_SuperFX(Word >> 8, Address + 1);
				break;
		}

		//if (o)
		S9xSetByte_SuperFX((uint8) Word, Address);
	...
	if (SetAddress >= (uint8 *) CMemory::MAP_LAST)
	{
		...
		addCyclesInMemoryAccess_x2_SuperFX;
	...
	switch ((pint) SetAddress)
	{
		case CMemory::MAP_CPU:
			/*
			   ...
			   {
			 */
			S9xSetCPU_SuperFX(Word >> 8, (Address + 1) & 0xffff);
			addCyclesInMemoryAccess_SuperFX;
			S9xSetCPU_SuperFX((uint8) Word, Address & 0xffff);
			addCyclesInMemoryAccess_SuperFX;
			return;
			//}
			/*
		...
		case CMemory::MAP_PPU:
			if (CPU.InDMAorHDMA)
			{
				if ((Address & 0xff00) != 0x2100)
					S9xSetPPU_SuperFX((uint8) Word, Address & 0xffff);
				if (((Address + 1) & 0xff00) != 0x2100)
					S9xSetPPU_SuperFX(Word >> 8, (Address + 1) & 0xffff);
				return;
			}
...
inline void S9xSetWord_SA1_Write10_WrapBank(uint16 Word, uint32 Address)
{
	uint32	mask = MEMMAP_MASK & (0xffff);
	if ((Address & mask) == mask)
	{
		...
		a.xPBPC = Address;
		a.W.xPC++;
		S9xSetByte_SA1(Word >> 8, a.xPBPC);

		S9xSetByte_SA1((uint8) Word, Address);
...
inline void S9xSetWord_SA1_Write10_WrapPage(uint16 Word, uint32 Address)
{
	uint32	mask = MEMMAP_MASK & 0xff;
	if ((Address & mask) == mask)
	{
...
inline void S9xSetWord_SA1_Write10 (uint16 Word, uint32 Address, enum s9xwrap_t w = WRAP_NONE)
{
	uint32	mask = MEMMAP_MASK & (w == WRAP_PAGE ? 0xff : (w == WRAP_BANK ? 0xffff : 0xffffff));
	if ((Address & mask) == mask)
	{
...
inline void S9xSetWord_Write10_WrapPage_NoSA1(uint16 Word, uint32 Address)
{
	uint32	mask = MEMMAP_MASK & (0xff);
	if ((Address & mask) == mask)
	{
		...
		a.xPBPC = Address;
		a.B.xPCl++;
		S9xSetByte_NoSA1(Word >> 8, a.xPBPC);

		S9xSetByte_NoSA1((uint8) Word, Address);
...
inline void S9xSetWord_Write10_WrapBank_NoSA1(uint16 Word, uint32 Address)
{
	uint32	mask = MEMMAP_MASK & (0xffff);
	if ((Address & mask) == mask)
	{
...
inline void S9xSetPCBase_SA1 (uint32 Address)
{
...
inline void S9xSetPCBase_SuperFX (uint32 Address)
{
	Registers.PBPC = Address & 0xffffff;
	ICPU.ShiftedPB = Address & 0xff0000;
...
inline void S9xSetPCBase_NoSA1 (uint32 Address)
{
	Registers.PBPC = Address & 0xffffff;
	ICPU.ShiftedPB = Address & 0xff0000;
...
inline uint8 * S9xGetBasePointer_SuperFX (uint32 Address)
{
	uint8	*GetAddress = Memory.Map[(Address & 0xffffff) >> MEMMAP_SHIFT];
...
inline uint8 * S9xGetMemPointer_SuperFX(uint32 Address)
{
	uint8	*GetAddress = Memory.Map[(Address & 0xffffff) >> MEMMAP_SHIFT];
