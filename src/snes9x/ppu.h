...
struct SPPU
{
	...
	uint8	MosaicStart;
	bool8	BGMosaic[4];
	bool8	DisableMosaicHackNonActive;
...
void S9xSetPPU_SuperFX (uint8, uint16);
uint8 S9xGetPPU (uint16);
uint8 S9xGetPPU_SuperFX (uint16);
void S9xSetCPU (uint8, uint16);
void S9xSetCPU_NoSA1 (uint8, uint16);
void S9xSetCPU_SuperFX (uint8, uint16);
uint8 S9xGetCPU (uint16);
uint8 S9xGetCPU_SuperFX (uint16);
...
static inline void FLUSH_REDRAW_SFX (void)
{
	if (IPPU.PreviousLine != IPPU.CurrentLine)
