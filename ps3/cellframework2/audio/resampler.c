...
#define SAMPLES_TO_FRAMES(x,y) ((x)/AUDIO_CHANNELS)
#define FRAMES_TO_SAMPLES(x,y) ((x)*AUDIO_CHANNELS)
...
int32_t resampler_cb_read(resampler_t *state, uint32_t frames, float *data)
{
   ...
   if (SAMPLES_TO_FRAMES(state->data_ptr, state) < req_frames)
   {
      ...
      while (has_read < must_read)
      {
         ...
         __builtin_memcpy(temp_buf + FRAMES_TO_SAMPLES(has_read, state), ptr, FRAMES_TO_SAMPLES(copy_size, state) * sizeof(float));
      ...
      __builtin_memcpy(state->data + state->data_ptr, temp_buf, FRAMES_TO_SAMPLES(must_read, state) * sizeof(float));
