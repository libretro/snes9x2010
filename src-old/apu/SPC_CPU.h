...
unsigned SNES_SPC::CPU_mem_bit( uint8_t const* pc, rel_time_t rel_time )
{
   ...
   unsigned t = CPU_READ ( rel_time, TIME_ADJ(0), (addr & 0x1FFF) ) >> (addr >> 13);
	return t << 8 & 0x100;
}
#endif
...
#define BRANCH( cond )\
	...
	case 0x6F:// RET
...
#else
		{
			...
			SET_PC( (sp [-0x101] << 8) + ram [(uint8_t) addr + 0x100] );
			sp -= 0x100;
		}
#endif
		goto loop;
	...
	case 0xC4: // MOV dp,a
...
#if !SPC_MORE_ACCURACY
		  {
			  ...
			  if ( (unsigned) i < 0x10 ) // 39%
			  {
				  ...
				  if ( sel == 1 ) // 51% $F3
				  {
					  RUN_DSP( rel_time, reg_rel_times [REGS [r_dspaddr]] )
#if SPC_LESS_ACCURATE
						  else if ( m.dsp_rel_time == skipping_rel_time )
						  {
							  int r = REGS [r_dspaddr];
							  if ( r == SPC_DSP::r_kon )
								  m.skipped_kon |= a & ~dsp.m.regs[SPC_DSP::r_koff];
						  ...
						  SPC_DSP_WRITE_HOOK( m.spc_rel_time + rel_time, REGS [r_dspaddr], (uint8_t) a );
#endif

						  if ( REGS [r_dspaddr] <= 0x7F )
						  {
							  int addr = REGS[r_dspaddr];
							  dsp.m.regs [addr] = (uint8_t) a;
							  switch ( addr & 0x0F )
							  {
								  case v_envx:
									  dsp.m.envx_buf = (uint8_t) a;
									  break;

								  case v_outx:
									  dsp.m.outx_buf = (uint8_t) a;
									  break;

								  case 0x0C:
									  if ( addr == r_kon )
										  dsp.m.new_kon = (uint8_t) a;

									  if ( addr == r_endx ) // always cleared, regardless of a written
									  {
										  dsp.m.endx_buf = 0;
										  dsp.m.regs [r_endx] = 0;
									  }
...
set_psw:
	...
	case 0x4A: // AND1 C,mem.bit
		   addr = READ_PC16( pc );
		   t = CPU_READ ( rel_time, TIME_ADJ(0), (addr & 0x1FFF) ) >> (addr >> 13);
		   c &= t << 8 & 0x100;
		   pc += 2;
		   goto loop;
	...
	case 0x0A: // OR1 C,mem.bit
		   addr = READ_PC16( pc );
		   t = CPU_READ ( rel_time + -1, TIME_ADJ(0), (addr & 0x1FFF) ) >> (addr >> 13);
		   c |= t << 8 & 0x100;
		   pc += 2;
		   goto loop;
