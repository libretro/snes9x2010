...
{
	uint16	val = S9xGetWord_WrapBank(Registers.PBPC);
	if (a & READ)
		OpenBus = (uint8) (val >> 8);
	Registers.PCw += 2;
...
static inline uint32 AbsoluteIndexedIndirectSlow (AccessMode a)			// (a,X)
{
	...
	uint16	addr2 = S9xGetWord_WrapBank(ICPU.ShiftedPB | addr);
	OpenBus = addr2 >> 8;
...
{
      uint32 addr = (!(CheckEmulation()) || Registers.DL) ? S9xGetWord_WrapBank(DirectSlow(READ)) : S9xGetWord_WrapPage(DirectSlow(READ));
...
{
	uint32	addr = Registers.DL ? S9xGetWord_WrapBank(DirectSlow(READ)) : S9xGetWord_WrapPage(DirectSlow(READ));
...
{
	uint32	addr = (!CheckEmulation() || Registers.DL) ? S9xGetWord_WrapBank(DirectIndexedXSlow(READ)) : S9xGetWord_WrapPage(DirectIndexedXSlow(READ));
...
{
	uint32	addr = Registers.DL ? S9xGetWord_WrapBank(DirectIndexedXE1(READ)) : S9xGetWord_WrapPage(DirectIndexedXE1(READ));
