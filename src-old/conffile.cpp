...
bool ConfigFile::LoadFile(const char *filename){
    ...
    if((s=OPEN_STREAM(filename, "r"))){
        ...
        n2=strrchr(n, '\\'); if(n2!=NULL) n=n2+1;
        LoadFile_freader(new fReader(s), n);
        CLOSE_STREAM(s);
        ret = true;
    }
    #if 0
...
void ConfigFile::LoadFile_freader(fReader *r, const char *name){
	curConfigFile = this;
    string l, key, val;
    string section;
    string comment;
...
bool ConfigFile::SaveTo(const char *filename, bool no_sections){
    string section;
    FILE *fp;
   ...
   {
		ConfigEntry e (k->line, k->section, k->key, k->val);
      e.comment = k->comment;
		data2.insert(e);
	}
#else
...
#if 0
void ConfigFile::LoadFile_unzreader(unzReader *r, const char *name){
	curConfigFile = this;
    string l, key, val;
    string section;
    string comment;
...
//only used for snes9x.cpp - ROM::Filename
