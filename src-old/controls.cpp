...
void S9xSetController (int port, int controller, int8 id1, int8 id2, int8 id3, int8 id4)
{
	if (port < 0 || port > 1)
		return;
...
void S9xGetController (int port, int *controller, int8 *id1, int8 *id2, int8 *id3, int8 *id4)
{
	int	i;
	...
	*id1 = *id2 = *id3 = *id4 = -1;
	...
	switch (i = newcontrollers[port])
	{
		case MP5:
			*controller = CTL_MP5;
			*id1 = (mp5[port][0] == NONE) ? -1 : mp5[port][0] - JOYPAD0;
			*id2 = (mp5[port][1] == NONE) ? -1 : mp5[port][1] - JOYPAD0;
			*id3 = (mp5[port][2] == NONE) ? -1 : mp5[port][2] - JOYPAD0;
			*id4 = (mp5[port][3] == NONE) ? -1 : mp5[port][3] - JOYPAD0;
			return;
...
char * S9xGetCommandName (s9xcommand_t command)
{
	...
	switch (command.type)
	{
		...
		case S9xAxisJoypad:
			...
			s += " T=";
			s += int((mul<1000>(command.axis.joypad.threshold + 1)) >> 8) / 10.0;
			s += "%";
		...
		case S9xAxisPseudobuttons:
			...
			s += " T=";
			s += int(mul<1000>(command.axis.button.threshold + 1) >> 8) / 10.0;
			s += "%";
	...
	if (!strcmp(name, "None"))
		...
		if (!strncmp(name, "Joypad", 6))
		{
			...
			if (!strncmp(name + 8, "Axis ", 5))
			{
				...
				if (i < 0)
					return (cmd);
				cmd.axis.joypad.threshold = ((i - 1) << 8) / 1000;
							...
							if (!strncmp(name, "ButtonToPointer ", 16))
							{
								...
								if (!strncmp(name, "AxisToPointer ", 14))
								{
									...
									if (!strncmp(name, "AxisToButtons ", 14))
									{
										...
										if (i < 0)
											return (cmd);
										cmd.axis.button.threshold = ((i - 1) << 8) / 1000;
...
//#define S9xGetMapping(id) keymap[id]
...
bool S9xMapButton (uint32 id, s9xcommand_t mapping, bool poll, s9xcommand_t * keymap)
{
	//int	t;
...
#if 0
void S9xReportButton (uint64 id, uint64 pressed)
{
	if (keymap.count(id) == 0)
		return;
...
bool S9xMapPointer (uint32 id, s9xcommand_t mapping, bool poll, s9xcommand_t * keymap)
{
	//int	t;
...
#define ApplyPointer(id, data1, data2, keymap) \
         s9xcommand_t cmd = keymap[id]; \
...
void S9xReportPointer (uint32 id, int16 x, int16 y, bool type, s9xcommand_t *keymap)
{
#if 0
	if (keymap.count(id) == 0)
		return;
	...
	ApplyPointer(id, x, y, keymap);
}

void S9xReportPointer (uint32 id, int16 x, int16 y, s9xcommand_t * keymap)
{
#if 0
	if (keymap.count(id) == 0)
		return;
...
void S9xApplyCommand_Button (s9xcommand_t cmd, uint64_t data1)
{
	int	i;
...
#if 0
		case S9xButtonJoypad:
			joypad[cmd.button.joypad.idx] = ((joypad[cmd.button.joypad.idx] | r) & (((data1) | -(data1)) >> 31)) | ((joypad[cmd.button.joypad.idx] & ~r) & ~(((data1) | -(data1)) >> 31));
#if 0
			if (cmd.button.joypad.toggle)
			{
				if (!data1)
			...
			else
			{
				...
				if (data1)
				{
					if (!(Movie.State == MOVIE_STATE_PLAY)) // if up+down isn't allowed AND we are NOT playing a movie,
						{
				...
				if (data1)
				{
					...
					switch ((enum command_numbers) (i = cmd.button.command))
					{
						...
						case CheatToggle:
							...
							sprintf(buf, "%s cheat: %d", S9xCheatEnabled(Settings.CurrentCheatPosition) ? "Enabled" : "Disabled", Settings.CurrentCheatPosition);
							S9xSetInfoString(buf);
							break;
						case CheatEnable:
							S9xEnableCheat(Settings.CurrentCheatPosition);
							Cheat.c[Settings.CurrentCheatPosition].saved = TRUE;
							S9xApplyCheats();
							sprintf(buf, "Enabled cheat: %d", Settings.CurrentCheatPosition);
							S9xSetInfoString(buf);
							break;
						case CheatDisable:
							S9xDisableCheat(Settings.CurrentCheatPosition);
							Cheat.c[Settings.CurrentCheatPosition].saved = TRUE;
							S9xApplyCheats();
							sprintf(buf, "Disabled cheat: %d", Settings.CurrentCheatPosition);
							S9xSetInfoString(buf);
							break;
						case CheatInput:
							sprintf(buf,emulator_input_cheat());
							S9xSetInfoString(buf);	
							break;
						case CheatInputLabel:
							sprintf(buf,emulator_input_cheatlabel());
							S9xSetInfoString(buf);	
							break;
						case IncCheat:
							Settings.CurrentCheatPosition++;
							sprintf(buf, "Cheat select: %d (%s)", Settings.CurrentCheatPosition, S9xGetCheatName(Settings.CurrentCheatPosition));
							S9xSetInfoString(buf);
							break;
						case SramWriteProtect:
							Settings.SRAMWriteProtect = !Settings.SRAMWriteProtect;
							sprintf(buf, "SRAM write protect turned %s", Settings.SRAMWriteProtect ? "on" : "off");
							S9xSetInfoString(buf);
							break;
						case DecCheat:
						...
						case IncSave:
							...
							sprintf(buf, "Save state slot changed to: %d", Settings.CurrentSaveStateSlot);
							printf("IncSave\n");
							S9xSetInfoString(buf);
							break;
						case DecSave:
						...
						case Pause:
							...
							S9xSetSoundMute(Settings.Paused);
...
uint8 S9xReadJOYSERn_SuperFX (int n)
{
	//int	i, j, r;
	int i;
	...
	uint8 bits;
	ISEL_IF8(bits, (OpenBus & ~3) | ((n == 1)),0x1c,0);
		...
		switch (i = curcontrollers[n])
		{
			case MP5:
				...
				for (i = 0; i < 2; i++, j++)
				{
					if (mp5[n][j] == NONE)
						continue;
					ISEL_IF8(bits,r >= 16,bits | 1 << i,bits | ((joypad[mp5[n][j] - JOYPAD0] & (0x8000 >> r)) ? 1 : 0) << i);
					/*
					   if (r >= 16)
					   bits |= 1 << i;
					   else
					   bits |= ((joypad[mp5[n][j] - JOYPAD0].buttons & (0x8000 >> r)) ? 1 : 0) << i;
					 */
				}
				return (bits);
...
void S9xDoAutoJoypad_SuperFX (void)
{
	int	i;
	...
	FLAG_LATCH = 1;
	//end of S9xSetJoypadLatch(1);

	//begin of S9xSetJoypadLatch
	//S9xSetJoypadLatch(0);
	// 1 written, 'plug in' new controllers now
	curcontrollers[0] = newcontrollers[0];
	curcontrollers[1] = newcontrollers[1];
	...
	FLAG_LATCH = 0;
	//end of S9xSetJoypadLatch(0)


	//0
	switch (i = curcontrollers[0])
	{
		case JOYPAD0:
		case JOYPAD1:
			read_idx[0][0] = 16;
			WRITE_WORD(Memory.FillRAM + 0x4218, joypad[i - JOYPAD0]);
			WRITE_WORD(Memory.FillRAM + 0x421c, 0);
			break;

		case MOUSE0:
			read_idx[0][0] = 16;
			WRITE_WORD(Memory.FillRAM + 0x4218, mouse[i - MOUSE0].buttons);
			WRITE_WORD(Memory.FillRAM + 0x421c, 0);
			break;

		default:
			WRITE_WORD(Memory.FillRAM + 0x4218, 0);
			WRITE_WORD(Memory.FillRAM + 0x421c, 0);
			break;
	}
	//1
	switch (i = curcontrollers[1])
	{
		case JOYPAD0:
		case JOYPAD1:
			read_idx[1][0] = 16;
			WRITE_WORD(Memory.FillRAM + 0x4218 + 2, joypad[i - JOYPAD0]);
			WRITE_WORD(Memory.FillRAM + 0x421c + 2, 0);
			break;

		case MOUSE0:
			read_idx[1][0] = 16;
			WRITE_WORD(Memory.FillRAM + 0x4218 + 2, mouse[i - MOUSE0].buttons);
			WRITE_WORD(Memory.FillRAM + 0x421c + 2, 0);
			break;

		default:
			WRITE_WORD(Memory.FillRAM + 0x4218 + 2, 0);
			WRITE_WORD(Memory.FillRAM + 0x421c + 2, 0);
			break;
	}
}

void S9xDoAutoJoypad_SA1 (void)
{
	int	i;
	...
	FLAG_LATCH = 1;
	//end of S9xSetJoypadLatch

	//begin of S9xSetJoypadLatch(0)
	//S9xSetJoypadLatch(0);
	// 1 written, 'plug in' new controllers now
	curcontrollers[0] = newcontrollers[0];
	curcontrollers[1] = newcontrollers[1];
	...
	switch (i = curcontrollers[0])
	{
		case MP5:
			...
			if (mp5[0][0] == NONE)
				...
				WRITE_WORD(Memory.FillRAM + 0x4218, joypad[mp5[0][0] - JOYPAD0]);

			if (mp5[0][1] == NONE)
				WRITE_WORD(Memory.FillRAM + 0x4218 + 4, 0);
			else
				WRITE_WORD(Memory.FillRAM + 0x4218 + 4, joypad[mp5[0][1] - JOYPAD0]);

			read_idx[0][FLAG_IOBIT(0) ? 0 : 1] = 16;
			break;
	...
	switch (i = curcontrollers[1])
	{
		case MP5:
			...
			if (mp5[1][0] == NONE)
				WRITE_WORD(Memory.FillRAM + 0x4218 + 2 + (0 << 2), 0);
			else
				WRITE_WORD(Memory.FillRAM + 0x4218 + 2 + (0 << 2), joypad[mp5[1][0] - JOYPAD0]);

			if (mp5[1][1] == NONE)
				WRITE_WORD(Memory.FillRAM + 0x4218 + 2 + (1 << 2), 0);
			else
				WRITE_WORD(Memory.FillRAM + 0x4218 + 2 + (1 << 2), joypad[mp5[1][1] - JOYPAD0]);

			read_idx[1][FLAG_IOBIT(1) ? 0 : 1] = 16;
			break;
...
void S9xDoAutoJoypad (void)
{
	...
	switch (i = curcontrollers[0])
	{
		case MP5:
			//j = FLAG_IOBIT(0) ? 0 : 2;
			//ISEL_IF32(j,Memory.FillRAM[0x4213] & 0x40,0,2);
		...
		case SUPERSCOPE:
			read_idx[0][0] = 16;
			Memory.FillRAM[0x4218] = 0xff;
			Memory.FillRAM[0x4219] = superscope.read_buttons;
			WRITE_WORD(Memory.FillRAM + 0x421c, 0);
			break;
		...
		case TWO_JUSTIFIERS:
			read_idx[0][0] = 16;
			WRITE_WORD(Memory.FillRAM + 0x4218, 0x000e);
			WRITE_WORD(Memory.FillRAM + 0x421c, 0);
			break;
	...
	switch (i = curcontrollers[1])
	{
		case MP5:
			//j = FLAG_IOBIT(1) ? 0 : 2;
			//ISEL_IF32(j,Memory.FillRAM[0x4213] & 0x80,0,2);

			if (mp5[1][0] == NONE)
				WRITE_WORD(Memory.FillRAM + 0x4218 + 2, 0);
			else
				WRITE_WORD(Memory.FillRAM + 0x4218 + 2, joypad[mp5[1][0] - JOYPAD0]);

			if (mp5[1][1] == NONE)
				WRITE_WORD(Memory.FillRAM + 0x4218 + 2 + 4, 0);
			else
				WRITE_WORD(Memory.FillRAM + 0x4218 + 2 + 4, joypad[mp5[1][1] - JOYPAD0]);
		...
		case SUPERSCOPE:
			read_idx[1][0] = 16;
			Memory.FillRAM[0x4218 + 2] = 0xff;
			Memory.FillRAM[0x4219 + 2] = superscope.read_buttons;
			WRITE_WORD(Memory.FillRAM + 0x421c + 2, 0);
			break;
		...
		case TWO_JUSTIFIERS:
			read_idx[1][0] = 16;
			WRITE_WORD(Memory.FillRAM + 0x4218 + 2, 0x000e);
			WRITE_WORD(Memory.FillRAM + 0x421c + 2, 0);
			break;
...
void S9xControlEOF_SuperFX (void)
{
	//int					i;
...
void S9xControlEOF (void)
{
	...
	{
#if 0
		for (j = 0, i = mp5[0][j]; j < 4; i = mp5[0][++j])
		{
			if (i == NONE)
				continue;
...
do_justifier:
	...
	{
#if 0
		for (j = 0, i = mp5[1][j]; j < 4; i = mp5[1][++j])
		{
			if (i == NONE)
				continue;
		...
		break;
		case SUPERSCOPE:
		if (!(superscope.phys_buttons & SUPERSCOPE_OFFSCREEN))
		{
			if (superscope.next_buttons & (SUPERSCOPE_FIRE | SUPERSCOPE_CURSOR))
				DoGunLatch(superscope.x, superscope.y);
...
#define COPY_PRE(x)	{ memcpy((char *) s->internal + i, &(x), sizeof(x)); i += sizeof(x); }
#define COPY_POST(x)	{ memcpy(&(x), (char *) s->internal + i, sizeof(x)); i += sizeof(x); }

#define COPYVARIABLES(COPY) \
   COPY(joypad[0]); \
   COPY(joypad[1]); \
   COPY(joypad[2]); \
   COPY(joypad[3]); \
   ...
   COPY(joypad[7]); \
   \
   COPY(mouse[0].delta_x); \
   COPY(mouse[0].delta_y); \
   COPY(mouse[0].old_x); \
   COPY(mouse[0].old_y); \
   COPY(mouse[0].cur_x); \
   COPY(mouse[0].cur_y); \
   COPY(mouse[0].buttons); \
   \
COPY(mouse[1].delta_x); \
COPY(mouse[1].delta_y); \
COPY(mouse[1].old_x); \
COPY(mouse[1].old_y); \
COPY(mouse[1].cur_x); \
COPY(mouse[1].cur_y); \
COPY(mouse[1].buttons); \
   \
COPY(superscope.x); \
COPY(superscope.y); \
...
void S9xControlPostLoadState (struct SControlSnapshot *s)
{
	...
	ISEL_IF16(justifier.buttons,s->justifier_select & 1,justifier.buttons | JUSTIFIER_SELECT,justifier.buttons & ~JUSTIFIER_SELECT);
	/*
