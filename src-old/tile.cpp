...
void S9xSelectTileRenderers_SuperFX_SubFalse()
{
...
void S9xSelectTileRenderers (int BGMode, bool8 sub, bool8 obj)
{
	...
	else
	{
      ...
      uint32_t bgmode_5_or_6 = BGMode == 5 || BGMode == 6;
		if (obj) // OBJ
		{
			hires = (bgmode_5_or_6 || IPPU.PseudoHires);
			interlace = IPPU.InterlaceOBJ;
		}
		else
      {
         hires = isel_if(bgmode_5_or_6, TRUE, IPPU.PseudoHires);
         interlace = isel_if(bgmode_5_or_6, IPPU.Interlace, FALSE);
      }
...
void S9xSelectTileConverter_HiResFalse_SubFalse(int depth)
{
	switch (depth)
	{
		case 8:
...
void S9xSelectTileConverter_HiResTrue_SubTrue (int depth)
{
	switch (depth)
	{
		case 8:
...
void S9xSelectTileConverter_HiResTrue_SubFalse(int depth, bool8 mosaic)
{
	switch (depth)
	{
		case 8:
...
{ \
	...
	for (uint32 p = 0; p < 8; p++) \
		for (uint32 c = 0; c < 256; c++) \
			DirectColourMaps[p][c] = (((int) (IPPU.XB[((c & 7) << 2) | ((p & 1) << 1)]) << 10) | ((int) (IPPU.XB[((c & 0x38) >> 1) | (p & 2)]) << 5) | (int) (IPPU.XB[((c & 0xc0) >> 3 ) | (p & 4)])); \
	...
	{ \
		...
		\
      starty = isel_if(PPU.Mode7VFlip,255 - (int) (Line + 1),Line + 1); \
		...
		\
      startx = isel_if(PPU.Mode7HFlip,Right - 1,Left); \
      aa = isel_if(PPU.Mode7HFlip,-l->MatrixA,l->MatrixA); \
      cc = isel_if(PPU.Mode7HFlip,-l->MatrixC,l->MatrixC); \
...
{ \
	...
	{ \
      ...
      starty = isel_if(PPU.Mode7VFlip,255 - (int) (Line + 1),Line + 1); \
		...
		int	BB = ((l->MatrixB * starty) & ~63) + ((l->MatrixB * yy) & ~63) + (CentreX << 8); \
		int	DD = ((l->MatrixD * starty) & ~63) + ((l->MatrixD * yy) & ~63) + (CentreY << 8); \
      startx = isel_if(PPU.Mode7HFlip,MRight - 1,MLeft); \
      aa = isel_if(PPU.Mode7HFlip,-l->MatrixA,l->MatrixA); \
      cc = isel_if(PPU.Mode7HFlip,-l->MatrixC,l->MatrixC); \
		int	xx = CLIP_10_BIT_SIGNED(HOffset - CentreX); \
		int	AA = l->MatrixA * startx + ((l->MatrixA * xx) & ~63); \
